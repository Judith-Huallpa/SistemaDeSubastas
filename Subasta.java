    ...
    public Lote getLote(int numeroDeLote){
        if( (numeroDeLote >=1)&& (numeroDeLote < numeroDeLoteSiguiente)){
            ...
            if(loteElegido.getNumero()!= numeroDeLote){
                System.out.println("Error interno: se retorno el lote No: "+ loteElegido.getNumero()+ " en luga de "+ numeroDeLote);
            }
            return loteElegido;
        }else{
            System.out.print("El lote numero: " + numeroDeLote + " no eciste");
            return null;
        }
    }
